 ...
 <body id="dt_example" class="master_form_img">
	<div class="container">
 		...
 		<div class="row header_margin_top">
			<div align="center" >
				  <h2 class="form-name style_heading">Product Billing Copy</h2>
			</div>
			</div>
          		...
          		<div class="col-sm-3" >
					...
					<%
				    	BillOfMaterialDao dao = new BillOfMaterialDao();
							List compTypeList = dao.getBillNo();
						%>
						...
						<datalist id="productBillList">
							<%
								...
								ProductBillingHibernateNew ctdh = (ProductBillingHibernateNew) compTypeList.get(i);
							%>
							<option data-value="<%=ctdh.getBillNo()%>" value="<%=ctdh.getBillNo()%>>><%=ctdh.getBillDate() %>"
							myvalue="<%=ctdh.getFkClientId()%>"
							myvalue1="<%=ctdh.getClientName()%>"
							myvalue2="<%=ctdh.getCGST()%>"
							myvalue3="<%=ctdh.getSGST()%>"
							myvalue4="<%=ctdh.getIGST()%>"
							myvalue5="<%=ctdh.getTotalTaxAmount()%>"
							myvalue6="<%=ctdh.getTotalSubTotal()%>"
							myvalue7="<%=ctdh.getGrossTotal()%>"
							myvalue8="<%=ctdh.getTaxPercentage()%>"
							>									
							<%
								}
							%>
	...
	<div align="center">					
		<div class="col-md-12">
			<input type="button" class="btn btn-success btn-lg btnn" value="Print" name="btn" onclick="validateGenerateBillCOPYForOtherBill()">			
		</div>
	</div>
