...
//response.setHeader("Content-disposition","attachment; filename = ResourcebillingCopy.pdf");
	String fkVendorId = (String)session.getAttribute("fkVendorId");
	String resourceBillNoForPdf = (String)session.getAttribute("resourceBillNoForPdf");
	System.out.println("RESOUCE BILLING PDF resourceBillNoForPdf :- " +resourceBillNoForPdf);
	...
	//String CGSTForPDF = (String) session.getAttribute("CGST");
	//String SGSTForPDF = (String) session.getAttribute("SGST");
...
Double gstAmt = 0.0;
	...
	try {
	 ...
	 /* 	//GETTING PRODUCT DATA FROM DATABASE
		...
		session3 = hbu.getHibernateSession();
		
		List<ResourceBillingBean> billList1=null;
		
		Query query21 = session3.createSQLQuery("select bill_period_start_date,bill_period_end_date from resource_billing WHERE bill_no = :resourceBillNoForPdf");
		query21.setParameter("resourceBillNoForPdf", resourceBillNoForPdf);
		
		List<Object[]> results21 = query21.list();
		billList1= new ArrayList<ResourceBillingBean>(0);
		
		for(Object[] o : results21)
		{
			ResourceBillingBean vendorBean1 = new ResourceBillingBean();
			
			
			vendorBean1.setBillPeriodStartDate((Date) o[0]);
			vendorBean1.setBillPeriodEndDate((Date)o[1]);
        ...
        table_cell  = new PdfPCell(new Phrase(""+resourceBillNoForPdf,Normalfont11));
		table_cell.setHorizontalAlignment(Element.ALIGN_RIGHT);
		table_cell.setBorder(Rectangle.NO_BORDER);
		headercn.addCell(table_cell);
...
// 		headercn.addCell(table_cell);
		...
		table_cell1001  = new PdfPCell(new Phrase("Tax Invoice ",font18Bold11));
	...
	//For Company Name
		...
		PdfPCell table_cell;
		    ...
		    table_cell  = new PdfPCell(new Phrase("Embel Technologies",font15Bold));
			table_cell.setHorizontalAlignment(Element.ALIGN_LEFT);
			table_cell.setBorder(Rectangle.NO_BORDER);
			headercn.addCell(table_cell);
...
//				headercn.addCell(table_cell);
		...
		int k=0;
	 	...
	 	{ */
	 		GetVendorDetailsBean sr=(GetVendorDetailsBean)list12.get(k);
...
// 			table1.addCell(table_cell1);
		...
		Query query2 = session2.createSQLQuery("select product_name, hsn_sac, unit_price, quantity, GST, IGST, tax_amount, total from resource_billing WHERE bill_no = :resourceBillNoForPdf");
		query2.setParameter("resourceBillNoForPdf", resourceBillNoForPdf);
